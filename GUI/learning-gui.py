...
class Window(): 
    ...
    def set_shape(self):
        ...
        self.canvas = Canvas(self.root, bg="blue", height=200, width=200)
        
        self.line = self.canvas.create_line(0,0,0,200, fill='white')
        self.canvas.pack()
        
        self._shape_exists = True
        del_shape_button = Button(self.root, text="Remove shape", command= lambda: (self.canvas.pack_forget(), del_shape_button.pack_forget(), move_it_button.pack_forget(), self.__setattr__("_shape_exists", False)))
